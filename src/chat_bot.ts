...
function readMessage(userMessage: string, role: string, memberDuration: number): string {
    ...
    for (let i = 0; i < botResponses.length; i++) {
        ...
        const confidence = getConfidence(score, role, memberDuration, totalKeywords);
...
function getConfidence(
    ...
    const roleBias = rolesData.find(roleData => roleData.role === role);
    if (roleBias) {
        score *= roleBias.bias;
    }
...
function main() {
  rl.question("Enter a message: ", (userInput) => {
    if (userInput) {
      const botResponse = readMessage(userInput, 'none', 0);
